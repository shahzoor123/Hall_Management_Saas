 ...
 */
function serverBareModulesPlugin({
  config,
  options
}) {
  // Resolve paths according to tsconfig paths property
  let matchPath = config.tsconfigPath ? tsconfig.createMatchPath(config.tsconfigPath) : undefined;
  ...
  return {
    ...
    setup(build) {
      build.onResolve({
        ...
        if (options.onWarning && !isNodeBuiltIn(packageName) && !/\bnode_modules\b/.test(importer) && (
        ...
        pkgManager === "npm" || pkgManager === "yarn" && process.versions.pnp == null)) {
          try {
            ...
            options.onWarning(`The path "${path$1}" is imported in ` + `${path.relative(process.cwd(), importer)} but ` + `"${path$1}" was not found in your node_modules. ` + `Did you forget to install it?`, path$1);
          }
        }
        ...
        if (options.onWarning && !isNodeBuiltIn(packageName) && kind !== "dynamic-import" && config.serverPlatform === "node") {
          warnOnceIfEsmOnlyPackage(packageName, path$1, importer, options.onWarning);
        }
...
function warnOnceIfEsmOnlyPackage(packageName, fullImportPath, importer, onWarning) {
  try {
